...
abstract class base extends html
{
    ...
    public function get_menu($result)
    {
        ...
        $items["menu_list"] = array(
            array(
                $url->get(array($result["meta_data"]["settings"]["app_space_name"], "guest/home.show", ""), array(), ""),
                "Home",
            ),
            array(
                $url->get(array($result["meta_data"]["settings"]["app_space_name"], "guest/article.list_all", ""), array(), ""),
                "Articles",
            ),
            array(
                $url->get(array($result["meta_data"]["settings"]["app_space_name"], "guest/category.list_all", ""), array(), ""),
                "Cateories",
            ),
            array(
                $url->get(array($result["meta_data"]["settings"]["app_space_name"], "guest/tag.list_all", ""), array(), ""),
                "Tags",
            ),
            array(
                $url->get(array($result["meta_data"]["settings"]["app_space_name"], "guest/about.show", ""), array(), ""),
                "About",
            ),
        );
